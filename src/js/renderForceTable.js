...
function getFT(name, body, userid=false){
    return `<h1>${name}</h1>
...
function getFTR(name, counts, successRate, breakRate){
    ...
    let pyucha=(per(counts.success,sum)-successRate).toFixed(2);
    let pyuchaCSS=(pyucha>0?"blue":"red")
    let tail=`<td style="color:${pyuchaCSS}" rowspan="${rowspan}"><span>${pyucha}</span></td>`
...
function renderStarforcePreventBreak(data){
    let innerHTML="";
    for(let i=15; i<20; i++)innerHTML += getFTR(i+"성", data[0][i], STARFORCE_SUCCESS_RATE[i]+STARFORCE_BREAK_RATE[i],0)
    wrapper.innerHTML= getFT("스타포스(파괴방지-코인)", innerHTML);
    innerHTML="";
    for(let i=20; i<data[1].length; i++)innerHTML += getFTR(i+"성", data[1][i], STARFORCE_SUCCESS_RATE[i]+STARFORCE_BREAK_RATE[i],0)
